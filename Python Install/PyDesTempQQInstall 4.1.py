...
class MainWindow(QWidget):
    def __init__(self):
        ...
        # 设置窗口显示图标(标题栏)
        icon_path = os.path.join(os.getcwd() + "\\Resourse\\Icon.ico")
        print(icon_path)
        ...
        self.check_box2 = QCheckBox("使用'启动器'启动")
        # 将check_box2设为禁止状态
    ...
    def install_thread(self, install_path, plugin_install_path):
        ...
        with open(os.path.join(install_path, "versions", "9.9.16-28971", "resources", "app", "app_launcher", # 更改版本到9.9.16-29927
                               "liteloader.h.js"), 'w') as f:
            f_install_path = "require(String.raw`" + plugin_install_path + "/LiteLoaderQQNT`);"
            print(f_install_path)
            f.write(f_install_path.replace("/", "\\"))
